...
2. Run `renv::restore()` in your R console in order to recreate the virtual environment for running this script.
3. This work was performed on R version 4.0.2.
<br/><br/>
...
mwe_obj %>% 
  head(30) %>% 
  knitr::kable(caption = "Top AZ Ingredients Collocations")
```
...
mwe_az %>% 
  head(30) %>% 
  knitr::kable(caption = "Top Curated AZ Ingredients Collocations")
```
<br/><br/>
...
```{r, results='asis'}
corpus_df %>% 
  select(brand_name, active_ingredient, inactive_ingredient) %>% 
  head() %>% 
  knitr::kable(caption = "Sample of Active and Inactive Ingredients Docs")
...
mwe_obj %>% 
  head(50) %>% 
  knitr::kable(caption = "Top Ingredients Collocations for All Manufacturers")
```
<br/><br/>
...
mwe_all %>% 
  head(50) %>% 
  knitr::kable(caption = "Top Curated Ingredients Collocations for All Manufacturers")
